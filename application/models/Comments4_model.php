...
class Comments4_model extends CI_Model {
    ...
    public function get_comments() {
        ...
        $query = $this->db->query("SELECT c.id, c.id_user, c.comment, a.nama FROM tb_comments4 AS c JOIN tb_akun AS a ON c.id_user = a.id WHERE c.parent_id = 0 and c.pertemuan = 3");
    ...
    foreach ($query->result() as $row) {
        ...
        $reply_query = $this->db->query("SELECT c.id, c.id_user, c.comment, a.nama FROM tb_comments4 AS c JOIN tb_akun AS a ON c.id_user = a.id WHERE c.parent_id = $comment_id");
    ...
    public function save_comment($data) {
        $this->db->insert('tb_comments4', $data);
    }
