...
class Pertemuan4 extends CI_Controller {
    public function __construct() {
        ...
        $this->load->model('Pertemuan4_model'); // Load the model
    }
    ...
    public function save_comment4() {
        ...
        if ($this->session->userdata('email') != '') {
            $data = array(
                ...
                'parent_id' => 0,
                'pertemuan' => 4 // Set parent_id as 0 for main comments
            );
        ...
        if ($this->session->userdata('email') != '') {
            ...
            if ($upload) {
                ...
                if ($this->upload->do_upload('upload')) {
                    ...
                    //setAlert
                    $this->Pertemuan4_model->insertHasilTugas($data);
                    //set flashdata
    ...
    public function hapusTugas($id) {
        if ($this->session->userdata('email') != '') {
            $file = $this->Pertemuan4_model->getHasilTugasById($id);
            $filename = $file['upload'];
            $pertemuan = $file['pertemuan'];

            $this->Pertemuan4_model->deleteHasilTugas($id);
            ...
            if ($upload) {
                ...
                if ($upload != $filelama) {
                    ...
                    if ($this->upload->do_upload('upload')) {
                        ...
                        $this->Pertemuan4_model->updateHasilTugas($id, $data);
                    } else {
