...
class Pertemuan2 extends CI_Controller {
    public function __construct() {
        ...
        $this->load->model('Pertemuan2_model'); // Load the model
    }
    ...
    public function save_comment2() {
        ...
        if ($this->session->userdata('email') != '') {
            $data = array(
                ...
                'parent_id' => 0,
                'pertemuan' => 2 // Set parent_id as 0 for main comments
            );
        ...
        if ($this->session->userdata('email') != '') {
            ...
            if ($upload) {
                ...
                if ($this->upload->do_upload('upload')) {
                    ...
                    //setAlert
                    $this->Pertemuan2_model->insertHasilTugas($data);
                    //set flashdata
    ...
    public function hapusTugas($id) {
        if ($this->session->userdata('email') != '') {
            $file = $this->Pertemuan2_model->getHasilTugasById($id);
            $filename = $file['upload'];
            $pertemuan = $file['pertemuan'];

            $this->Pertemuan2_model->deleteHasilTugas($id);
            ...
            if ($upload) {
                ...
                if ($upload != $filelama) {
                    ...
                    if ($this->upload->do_upload('upload')) {
                        ...
                        $this->Pertemuan2_model->updateHasilTugas($id, $data);
                    } else {
